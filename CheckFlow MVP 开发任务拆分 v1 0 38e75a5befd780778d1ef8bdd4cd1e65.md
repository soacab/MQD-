...
## 11.3 VDrive 候选文件识别逻辑

```
1. 根据 project.vdrive_folder_id 作为根目录。
2. 调用 VDrive 接口读取文件元数据。
3. 按检查项自动执行配置识别候选文件。
4. 写入 auto_check_candidate_files。
5. 候选文件为空、多候选或 VDrive 异常时转入待选文件或人工判断。
```
...
```
1. 可以基于 VDrive 元数据识别检查项候选文件。
2. 可以保存候选文件的文件名、版本、时间、大小、路径和匹配依据。

...
```
一个业务检查项最多配置一条启用的自动检查执行规则。
file_content 类型配置内部表达“先完成文件存在性识别，再下载/解析/判断内容”的复合流程。
不是所有自动检查都需要 LLM。
...
一期不保存完整 VDrive 目录扫描现场，不落 VDrive 扫描批次、文件夹快照和全量文件快照。

系统只保存每个检查项实际识别到的候选文件，记录在 `auto_check_candidate_files`：
...
```
这样可以回答“某个检查项当时找到了哪些候选文件、最后选了哪个文件、依据是什么”。
如果未来需要追溯整个 VDrive 目录现场，再单独设计目录快照能力。
```
...
```
一期包含文件内容解析，file_parse_jobs 需要落表。
不保存完整 VDrive 目录快照；只保存检查项候选文件。
```
...
14. VDrive 不保存完整目录快照，只保存检查项候选文件和判断依据。
```
